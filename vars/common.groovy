...
def codeQuality() {
    ...
    withCredentials([usernamePassword(credentialsId: 'SONAR', passwordVariable: 'sonarPass', usernameVariable: 'sonarUser')]) {
         sh '''           
         #sonar-scanner -Dsonar.host.url=http://172.31.6.171:9000 -Dsonar.login=${sonarUser} -Dsonar.password=${sonarPass} -Dsonar.projectKey=${COMPONENT} -Dsonar.qualitygate.wait=true ${SONAR_EXTRA_OPTS}
         echo OK
        '''
    }
  }
...
def artifacts() {
    if ( env.TAG_NAME ==~ ".*" ) {
        ...
        stage('Prepare Artifacts') {
           ...
           sh """
            npm install
            zip -r ${COMPONENT}-${TAG_NAME}.zip node_modules.js server.js
           """
          }
          if (env.APPTYPE == "java") {
           sh """
        ...
        withCredentials([usernamePassword(credentialsId: 'NEXUS', passwordVariable: 'nexusPass', usernameVariable: 'nexusUser')]) {
            sh """
             curl -v -u ${nexusUser}:${nexusPass} --upload-file ${COMPONENT}-${TAG_NAME}.zip http://nexus.vykunta.internal:8081/repository/${COMPONENT}/${COMPONENT}-${TAG_NAME}.zip
          """
            }
        }
    }
